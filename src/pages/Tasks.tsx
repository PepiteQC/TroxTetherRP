...
import { Label } from "@nodelab/components/ui/label";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@nodelab/components/ui/select";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@nodelab/components/ui/form";
      ...
      <ResizablePanel defaultSize={60} className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-auto p-4 space-y-3 custom-scrollbar bg-black/90">
          {tasksLoading ? (
            ...
            tasks?.map(task => {
              const agentColor = agents?.find(a => a.id === task.agentId)?.color || "#00ff41";
