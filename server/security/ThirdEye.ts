...
import type {
  AlertLevel, AgentName, AgentTelemetry, AgentResult, AgentScore,
  ThirdEyeAlert, ThirdEyeAssurance, TroxtTaskPacket,
} from './types.js';
...
export class ThirdEye {
  ...
  constructor(private readonly scoreHistory: AgentScoreHistory) {}

  // ── 1. Observation globale ────────────────────────────────────────────────

  observe(telemetry: Record<string, AgentTelemetry>): ThirdEyeAlert[] {
    this.observationCount++;
    const newAlerts: ThirdEyeAlert[] = [];

    for (const [agentName, t] of Object.entries(telemetry)) {
      const agent = agentName as AgentName;
      if (t.status === 'blocked') {
        newAlerts.push(this.createAlert('BLACK',
          `${agent} est BLOQUÉ — production stoppée`,
          'Données corrompues si production forcée',
          `Résoudre les dépendances de ${agent} avant de continuer`,
          agent, [agent]
        ));
      }
      if (t.riskLevel === 'critical') {
        newAlerts.push(this.createAlert('RED',
          `${agent} signale un risque critique`,
          'Instabilité système possible',
          `Inspection immédiate de ${agent} par Ether-Lens`,
          agent, [agent]
        ));
      }
      if (t.confidence < 0.65 && t.status === 'working') {
        newAlerts.push(this.createAlert('ORANGE',
          `${agent} travaille avec une confiance faible (${Math.round(t.confidence * 100)}%)`,
          'Résultat potentiellement incomplet ou incorrect',
          `Demander un audit Ether-Lens sur la tâche "${t.taskId}"`,
          agent, [agent]
        ));
      }
    }
  ...
  predict(completedResults: AgentResult[], pendingPackets: TroxtTaskPacket[]): string[] {
    ...
    for (const packet of pendingPackets) {
      ...
      const missingDeps = deps.filter(dep =>
        !completedResults.some(r => r.taskId === dep || String(r.output.mission ?? '').includes(dep))
      );
      if (missingDeps.length > 0) {
        predictions.push(
          `⚠️ "${packet.mission}" attend ${missingDeps.join(', ')} — blocage probable`
        );
      }
    }

    const hasFactory = pendingPackets.some(p => p.targetAgent === 'ForgeFactory');
    const hasNamingValidation = completedResults.some(r =>
      String(r.output.namingStandard ?? '').length > 0
    );
  ...
  react(telemetry: Record<string, AgentTelemetry>): { agent: AgentName; signal: string; reason: string }[] {
    const reactions: { agent: AgentName; signal: string; reason: string }[] = [];
  ...
  private pushAlerts(alerts: ThirdEyeAlert[]): void {
  ...
  getAlerts(level?: AlertLevel, includeResolved = false): ThirdEyeAlert[] {
  ...
  getStatus() {
    ...
    const systemLevel: AlertLevel = criticalCount > 0 ? 'RED'
  ...
  private buildRecommendation(riskScore: number, warnings: string[]): string {
    if (riskScore === 0) return 'proceed';
    if (riskScore < 30) return 'proceed_with_guard_validation';
    if (riskScore < 60) return 'pause_and_add_missing_agents';
